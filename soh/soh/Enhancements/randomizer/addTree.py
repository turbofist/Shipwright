...
def add_to_location_list(file_path, values):
    pattern = (
        "locationTable[{0}] = Location::Tree({0}, RCQUEST_BOTH, RCAREA_HYRULE_FIELD, SCENE_HYRULE_FIELD, TWO_ACTOR_PARAMS({1}, {2}), \"{3}\", \"{4}\", RHT_TREE_HYRULE_FIELD, RG_GREEN_RUPEE, "
        "SpoilerCollectionCheck::RandomizerInf({5}));\n"
    ).format(*values)
...
def main():
    ...
    while True:
        ...
        add_to_location_list(
            location_list_file,
            [rc_name, actor_param1, actor_param2, description, short_name, rand_name],
        )
